...
public class SearchPageController extends AbstractSearchPageController
{
	...
	public String textSearch(@RequestParam(value = "text", defaultValue = "") final String searchText,
			@RequestParam(value = "pageSize", defaultValue = "0") final int pageSize, final HttpServletRequest request,
			final Model model) throws CMSItemNotFoundException
	{
		...
		if (StringUtils.isNotBlank(searchText))
		{
			...
			final PageableData pageableData = createPageableData(0, selectedPageSize, null, ShowMode.Page);
			...
			try
			{
				...
				{
					if (hpeWishlistFacade.isBookmarked(product.getCode()))
					{
						product.setBookmarkFlag(true);
					}
					else
					{
						product.setBookmarkFlag(false);
					}
				}
			}
			...
			else
			{
				...
				populateLeastPartnerPrices(searchPageData);
			...
			if (searchPageData != null)
			{
				model.addAttribute(WebConstants.BREADCRUMBS_KEY, hpeSearchBreadcrumbBuilder.getBreadcrumbs(null, searchText,
			...
			final Model model) throws CMSItemNotFoundException
	{
		...
		model.addAttribute(WebConstants.BREADCRUMBS_KEY, hpeSearchBreadcrumbBuilder.getBreadcrumbs(null, searchPageData));
	...
	protected void populateLeastPartnerPrices(final ProductSearchPageData<SearchStateData, ProductData> searchPageData)
	{
		final List<ProductData> productDataList = searchPageData.getResults();
		for (final ProductData productData : productDataList)
		{
			populateLeastPartnerPrice(productData);
		}
	}

	private void populateLeastPartnerPrice(final ProductData productData)
	{
		try
		{
			final PriceData priceData = hpeProductFacade.getProductLeastPartnerPrice(productData.getCode());
			if (priceData != null && priceData.getValue().doubleValue() > 0.0)
			{
				productData.setLeastPartnerPrice(priceData);
			}
		}
