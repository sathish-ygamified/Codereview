...
public class ProductPageController extends AbstractPageController
{
			...
			throws CMSItemNotFoundException, UnsupportedEncodingException
	{
		...
		// OCA call response value setting
		...
		{
			model.addAttribute("requestTime", productResult.get("requestTime"));
			model.addAttribute("responseTime", productResult.get("responseTime"));
			model.addAttribute("payloadSize", productResult.get("payloadSize"));
			model.addAttribute("productHierarchy", productResult.get("productHierarchy"));
			model.addAttribute("authStart", productResult.get("authStart"));
			model.addAttribute("authEnd", productResult.get("authEnd"));
			model.addAttribute("call1Start", productResult.get("call1Start"));
			model.addAttribute("call1End", productResult.get("call1End"));
			model.addAttribute("call2Start", productResult.get("call2Start"));
			model.addAttribute("call2End", productResult.get("call2End"));
		}
		JaloSession.getCurrentSession().setAttribute("productResult", null);
		...
		if (currentUser != null)
		{
			...
			model.addAttribute("customerEmailAddress", currentUser.getEmail());
		}
		...
		{
			model.addAttribute("listOfBundleOffer", listOfBundleOffer.get(0));
			model.addAttribute("bundlePrice", listOfBundleOffer.get(1));
			model.addAttribute("shopList", listOfBundleOffer.get(2));
		}
		...
		final String metaKeywords = MetaSanitizerUtil.sanitizeKeywords(productData.getKeywords());
		{
			...
			final JSONObject productJsonData = getProductDataJsonObjectForAnalytics(productData);
			model.addAttribute("productJsonData", productJsonData);
			...
			final List<ProductOption> extraOptions) throws CMSItemNotFoundException
	{
		...
		hpeProductFacade.setBadgeData(productModel, productData);
	...
	public @ResponseBody String channelCentralProductConfiguration(@RequestParam("variantCode")
	final String variantCode, @RequestParam("shopName")
	final String shopName, final Model model)
			throws CMSItemNotFoundException, JsonGenerationException, JsonMappingException, IOException
	{
		LOG.debug("HPEProductPageController :: channelCentralProductConfig()=" + variantCode + "-----" + shopName);
		String channelCentralJson = null;
		channelCentralJson = hpeProductFacade.getChannelCentralConfiguration(variantCode, shopName);
		return channelCentralJson;
	}

	private JSONObject getProductDataJsonObjectForAnalytics(final ProductData productData)
	{
		final JSONObject jsonObject = new JSONObject();
		final HashMap<String, Object> productInfo = new HashMap();
		productInfo.put("productID", productData.getCode());
		productInfo.put("productName", productData.getName());
